...
def printResult(train_x, train_y, _lambda, px_prob, iterations):
    ...
    for i, c in enumerate(labels):
        ...
        cmx[i][0][0] = np.sum(table) - np.sum(table[:, c]) - np.sum(table[i, :]) + table[i][c]    # tn
    ...
    for c in range(10):
        ...
        print("Is number {}             {}                  {}".format(c, cmx[c][1][1], cmx[c][1][0]))
        print("Isn\'t number {}         {}                  {}\n".format(c, cmx[c][0][1], cmx[c][0][0]))
        print('Sensitivity (Successfully predict number {})    : {:.5f}'.format(c, cmx[c][1][1]/ (cmx[c][1][1] + cmx[c][1][0])))
        print('Specificity (Successfully predict not number {}): {:.5f}\n\n'.format(c, cmx[c][0][0]/ (cmx[c][0][0] + cmx[c][0][1])))
    print("Total iteration to converge: {:d}".format(iterations))
    print("Total error rate: {}".format(float(train_x.shape[0] - total_correct) / train_x.shape[0]))
...
def EM(train_x, max_cnt=50):
    ...
    while(cnt <= max_cnt):
        ...
        diff = np.sum(np.abs(new_px_prob - px_prob)) + np.sum(np.abs(new_lambda - _lambda))
